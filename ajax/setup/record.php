<?php
	 ...
	 * Page database connexion
	 ====================================================================*/	
	require('../../'.__MAGICTREE_APPLICATION_RELEASE__.'/includes/common/db_connect.php');
	/*===================================================================*/	
	...
	$valeur = mysql_protect($_POST["valeur"]);

	//==================================================================	
	// Update setup value
	//==================================================================	
	$query = "
					UPDATE `".$_SESSION['iknow'][$ssid]['struct']['tb_configuration']['extension']."`
						SET `value` = '".$valeur."'
					WHERE
	...
	if(!isset($row['output']))
	{
		...
		$query = "
				INSERT 
				INTO`".$_SESSION[$ssid]['MT']['iksetup']->tree_caption."`
					(
					 `application_release`,
					 `id`,
					 `language`,
	...
	{
		$query = "
					UPDATE `".$_SESSION[$ssid]['MT']['iksetup']->tree_caption."`
	...
	if($num_rows == 0)
	{
		echo $_SESSION[$ssid]['message'][3];
	}
	else
	{
		echo $_SESSION[$ssid]['message'][2];
	}
?>