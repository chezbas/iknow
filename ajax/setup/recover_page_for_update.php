<?php
	...
	$query = "	SELECT 
				...
				FROM `".$_SESSION['iknow'][$ssid]['struct']['tb_configuration']['extension']."` `MTE`
			WHERE
				`MTE`.`application_release` = '".__MAGICTREE_APPLICATION_RELEASE__."'
				 AND `MTE`.`id` = ".$_SESSION[$ssid]['current_read_page']."
			";

	$result = $link_mt->query($query);
	$row = $result->fetch_array(MYSQLI_ASSOC);
	$input = '<input id ="my_value" size="180" value= '.$row['valeur'].'>';
	//==================================================================	
		
	$retour = array("HTML" => $html,"INPUT" => $input);
