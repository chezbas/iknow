...
// Contributors
//==================================================================
function move_contributor()
{
	try
	{
		document.getElementById('contributor_body').style.marginTop = contributor_offset+"px";
		contributor_offset = contributor_offset - 1;

		var height_text = document.getElementById('contributor').offsetHeight;
		if(contributor_offset < -2*height_text)
		{
			contributor_offset = contributor_offset_init;
		}
...
function resize_details()
{
	...
	if(document.getElementById('my_test') != undefined)
	{
		document.getElementById('my_test').style.height = document.getElementById('main_details').offsetHeight - document.getElementById('footer').offsetHeight - 5 + "px";
	}
...
function checklogin(language,mode,id_row)
{
	...
	if(mode)
	{
		var href = my_root_path+'/includes/security/home.php?ssid='+ssid+'&IKLNG='+language+'&id='+id_row;
	}
	else
	{
		var href = my_root_path+'/index.php'+'?IKLNG='+language+'&id='+id_row;
	}
	window.location = href;
}
