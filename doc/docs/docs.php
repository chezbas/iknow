...
function get_chemin($child_original){
...
Ext.BLANK_IMAGE_URL = 'doc/docs/resources/s.gif';
...
MainPanel = function(){
	...
	<?php 
		...
		if(isset($_GET['ID']))
		{
			...
			$titre = get_titre($_GET['ID']); 
		
            if($titre != false)
            {	
            	$existe = true;		// l'ID existe
            }
            else
            {    			
            	$existe = false;	// l'ID n'existe pas
            }			
...
?> 
    ...
    MainPanel.superclass.constructor.call(this, {
        ...
        items: {
            id:'welcome-panel',
            title: 'Portail',
            autoLoad: {url: 'doc/docs/accueil.php?ssid='+ssid, callback: this.initSearch, scope: this},
            iconCls:'icon-home',
            autoScroll: true
        }
    });
  <?php		
		if(isset($existe) && $existe == true && isset($_GET['ID']) && $_GET['ID'] != 1){
			$chemin = get_chemin($_GET['ID']);
			?>
			
			    var autoLoad = {url: 'doc/docs/affiche.php?ID=<?php echo $_GET['ID']; ?>&ssid=<?php echo $ssid; ?>'};

  			  var p = this.add(new DocPanel({
                    id: '<?php echo $chemin; ?>',
                    cclass : '<?php echo $chemin; ?>',
                    autoLoad: autoLoad,
                    iconCls: undefined,
                    titre: '<?php echo mysql_escape_string(get_titre($_GET['ID'])); ?>'
		...
		if(isset($_GET['ID']) && $existe == false)
		{
			
			?>
			
			    var autoLoad = {url: 'doc/docs/not_exist.php'};
...
Ext.extend(MainPanel, Ext.TabPanel, {
    ...
    loadClass : function(href, cls, member,titre){
        ...
        if(tab){
           ...
           /* alert(id);	// docs-Ext.chart.Axis
            alert(cls); // Ext.chart.Axis
            alert(autoLoad); // objet
            alert(Docs.icons[cls]); // undefined*/
            ...
            //p.on("close",function(){ tinyMCE.execCommand('mceRemoveControl', false, 'matiny');});		// Executé lors de la fermeture de l'onglet
            //p.on("close",function(){ alert('XD');});		// Executé lors de la fermeture de l'onglet
...
Ext.onReady(function(){
    ...
    api.on('click',function(node, e){
         ...
         if(node.isLeaf()){
            ...
            e.stopEvent();
           	...
           	//alert('avant');
            mainPanel.loadClass(node.attributes.href, node.id,undefined,node.attributes.text);

			verif_chargement = setInterval('verif_chargemement_page("'+node.attributes.href+'");',200);
    ...
    <?php 
    	...
    	if(isset($existe) && $existe)
    	{
    		...
    		echo 'api.expandPath(\'/root/apidocs\');';	// Path complet
    		
    	}
    	else
    	{
    		
    		echo 'api.expandPath(\'/root/apidocs\');';
