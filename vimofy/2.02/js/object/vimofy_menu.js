function vimofy_menu(id_vimofy) 
{ 
    this.id_vimofy = id_vimofy;
    ...
    { 
		...
		this.html += '<table id="table_header_menu_'+this.id_vimofy+'" class="__'+eval('Vimofy.'+this.id_vimofy+'.theme')+'_column_header_menu">';

    	for(j = 1;j <= this.qtt_line;j++)
    	{
    		if(eval('this.line.l'+j+'.type') == "LINE")
    		{
	    		this.html += '<tr class="__'+eval('Vimofy.'+this.id_vimofy+'.theme')+'_column_header_menu" '+eval('this.line.l'+j+'.event')+' id="'+this.menu_id+'_l'+j+'" onmouseover="Vimofy.'+this.id_vimofy+'.obj_menu.onmouseover(this,'+j+',\''+eval('this.parent')+'\',\''+this.menu_id+'_'+j+'\');">';
	    		this.html += '<td class="__'+eval('Vimofy.'+this.id_vimofy+'.theme')+'_column_header_menu_icon"><div class="'+eval('this.line.l'+j+'.class_icon')+'"></div></td>';
	    		this.html += '<td class="__'+eval('Vimofy.'+this.id_vimofy+'.theme')+'_column_header_menu_sep"></td>';
	    		this.html += '<td class="__'+eval('Vimofy.'+this.id_vimofy+'.theme')+'_column_header_menu_lib">'+eval('this.line.l'+j+'.title')+'</td>';
	    		if(eval('this.line.l'+j+'.child') != undefined)
	    		{
	    			// The line is parent, display an arrow
	    			this.html += '<td><div id="arrow_'+this.menu_id+'_l'+j+'" class="__'+eval('Vimofy.'+this.id_vimofy+'.theme')+'_ico __'+eval('Vimofy.'+this.id_vimofy+'.theme')+'_ico_arrow_menu_child"></div></td>';
	    		}
	    		else
	    		{
	    			this.html += '<td></td>';
    		...
    		{
    			// Line separator
	    		this.html += '<tr class="__'+eval('Vimofy.'+this.id_vimofy+'.theme')+'_column_header_menu vimofy_line_separator">';
	    		this.html += '<td class="__'+eval('Vimofy.'+this.id_vimofy+'.theme')+'_column_header_menu_icon_sep"></td>';
	    		this.html += '<td colspan=3 class="__'+eval('Vimofy.'+this.id_vimofy+'.theme')+'_column_header_menu_line_sep_top"></td>';
	    		this.html += '</tr>';
	    		this.html += '<tr class="__'+eval('Vimofy.'+this.id_vimofy+'.theme')+'_column_header_menu vimofy_line_separator">';
	    		this.html += '<td class="__'+eval('Vimofy.'+this.id_vimofy+'.theme')+'_column_header_menu_icon_sep"></td>';
	    		this.html += '<td colspan=3 class="__'+eval('Vimofy.'+this.id_vimofy+'.theme')+'_column_header_menu_line_sep_bottom"></td>';
	    		this.html += '</tr>';
    		}
    	}
    ...
    this.add_line = function(title,class_icon,event,enable,child)
    {
        ...
        if(enable)
        {
        	eval('this.line.l'+this.qtt_line+'.event = "onclick=\\"vimofy_toggle_header_menu(\''+this.id_vimofy+'\','+eval('Vimofy.'+this.id_vimofy+'.menu_opened_col;')+');'+event+'\\";"');
        ...
        if(typeof(child) != 'undefined')
        {
        	...
        	eval('this.line.l'+this.qtt_line+'.event = "onclick=\\"Vimofy.'+this.id_vimofy+'.obj_menu.display_child(this,'+this.qtt_line+',\''+this.menu_id+'\'); /**/\\" onmouseout=\\"Vimofy.'+this.id_vimofy+'.obj_menu.onmouseout('+this.qtt_line+');\\""');
    ...
    this.display_child = function(el,line_id,menu_id)
    {
    	...
    	if(this.opened_child != menu_id+'_'+line_id)
    	{
	    	...
	    	document.getElementById('arrow_'+menu_id+'_l'+line_id).className = '__'+eval('Vimofy.'+this.id_vimofy+'.theme')+'_ico __'+eval('Vimofy.'+this.id_vimofy+'.theme')+'_ico_arrow_menu_child_hover';
	
	    	eval('Vimofy.'+this.id_vimofy+'.stop_click_event = true');
	    	this.place_child();
	    	
	    	
	    	
	    	//alert((vimofy_getPosition(this.opened_child)));
    	}
    };
    ...
    this.hide_child = function(line,el)
    {
    	...
    	document.getElementById('arrow_'+this.opened_child_parent+'_l'+line).className = '__'+eval('Vimofy.'+this.id_vimofy+'.theme')+'_ico __'+eval('Vimofy.'+this.id_vimofy+'.theme')+'_ico_arrow_menu_child';
    	clearTimeout(this.hover_tempo);
    ...
    this.onmouseover = function(el,line,parent_menu,submenu)
    {
    	...
    	{
    		// This is a parent
    		this.hover_tempo = setInterval('Vimofy.'+this.id_vimofy+'.obj_menu.display_child(null,'+line+',\''+this.menu_id+'\');',500);
    	}
    	else
    	{
    ...
    {
    	// Vertical alignment
    	document.getElementById(this.opened_child).style.top = (this.el.offsetTop)-document.getElementById('vim_column_header_menu_'+this.id_vimofy).offsetHeight+'px';
    	
    	// Display the menu
    	document.getElementById(this.opened_child).style.display = 'block';
    	
    	// Get vimofy position
    	vimofy_pos = vimofy_getPosition('vim__'+eval('Vimofy.'+this.id_vimofy+'.theme')+'__vimofy_table_'+this.id_vimofy+'__');
    	
    	// Get vimofy width
    	vimofy_width = document.getElementById('vim__'+eval('Vimofy.'+this.id_vimofy+'.theme')+'__vimofy_table_'+this.id_vimofy+'__').offsetWidth;
    	
    	// Get menu position
    	menu_pos = vimofy_getPosition(this.el.id);
    	...
    	if((vimofy_width-(menu_pos[0]-vimofy_pos[0])-menu_width)<submenu_width)
    	{
    		// no enter
