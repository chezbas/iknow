		...
		else
		{
			if(evt.keyCode == 40)
			{
				...
				if(eval('Vimofy.'+vimofy_id+'.menu_quick_search') && eval('Vimofy.'+vimofy_id+'.input_search_selected_line') < 6)
				{
					...
					var test_div = document.getElementById('vimofy_input_result_'+eval('Vimofy.'+vimofy_id+'.input_search_selected_line + 1')+'_'+vimofy_id);
					...
					else
					{
						...
						if(evt.keyCode == 13)
						{
							...
							if(eval('Vimofy.'+vimofy_id+'.edit_mode') == __EDIT_MODE__)
							{
								//alert('a');
								//eval('clearTimeout(Vimofy.'+vimofy_id+'.time_input_search);');
								document.getElementById('th_input_'+column+'__'+vimofy_id).blur();
								save_lines(vimofy_id);
							}
							else
							{
